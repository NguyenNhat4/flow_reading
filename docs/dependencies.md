...
| File picking | `file_picker` 12.0.0-beta.7 (temporarily pinned) | MIT | Maintained Android Storage Access Framework integration and extension filtering. This prerelease fixes Android plugin registration with AGP 9 while Flutter's legacy Kotlin compatibility mode is enabled. | Milestone 1 selects EPUB candidates; content is copied immediately into app-private storage. Keep the exact prerelease pin until a stable 12.x release contains the AGP 9 fix and passes `flutter build apk --debug`. |
