...
void main() {
  test('detects and normalizes a general BCP-47 language', () async {
    ...
    final service = BookLanguageDetectionService(detector);
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        BookLanguageDetectionService.maximumSampleLength,
      );
    },
  );

  test('detector failure falls back to declared metadata language', () async {
    final service = BookLanguageDetectionService(_FakeDetector.failure());
  ...
  test('short or undetermined text does not block import', () async {
    final service = BookLanguageDetectionService(_FakeDetector('und'));
